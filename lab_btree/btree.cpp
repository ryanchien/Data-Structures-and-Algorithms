...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
		if (!subroot->elements.empty() && first_larger_idx < subroot->elements.size() && subroot->elements[int(first_larger_idx)].key == key)
				return subroot->elements[int(first_larger_idx)].value;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */	
		...
		else if (!subroot->elements.empty() && first_larger_idx < subroot->elements.size() && subroot->elements[int(first_larger_idx)].key == pair.key)
				return;
		else {
			insert(subroot->children[int(first_larger_idx)], pair);
			if ((subroot->children[int(first_larger_idx)])->elements.size() > order-1)
