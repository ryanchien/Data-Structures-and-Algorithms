...
size_t insertion_idx_helper(const std::vector<T>& elements, const C& val, size_t frnt, size_t bck)
{
		size_t mid = std::ceil((frnt+bck)/2);
		if (val == elements[mid]) //exact match
				return mid;
		else if (val > elements[mid] && val < elements[mid+1]) //1 after
				return mid+1;
		else if (val < elements[mid]) //lower half
				return insertion_idx_helper(elements, val, frnt, mid);
		else //upper half
				return insertion_idx_helper(elements, val, mid+1, bck);
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    /* TODO Your code goes here! */
		if (elements.size() == 0)
			...
			return 0;
		else if (val == elements[elements.size()-1])
