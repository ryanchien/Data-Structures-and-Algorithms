...
class BinaryTree
{
    public:
        ...
        bool isOrderedRecursive() const;
				bool isOrderedHelper(Node * node, T prev, T next) const;
        ...
        void getPaths(vector<vector<T> > &paths) const;
				void getPathsHelper(Node * node, vector<T> &path, vector<vector<T> > &paths) const;
        /**
        ...
        int sumDistances() const;
				int sumDistancesHelper(Node * node, int height) const;
        /**
