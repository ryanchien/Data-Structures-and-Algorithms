...
T sum(stack<T>& s)
{
	...
	T top = s.top();
	s.pop();
    T sumT = top + sum(s); // stub return value (0 for primitive types). Change this!
    s.push(top);            // Note: T() is the default value for objects, and 0 for
    return sumT;           // primitive types
}
...
void scramble(queue<T>& q)
{
	...
	int total_counter = 0;
	while(!q.empty()) {
		if (count_entries > (int)q.size()) //prevent index out of bounds
			count_entries = (int)q.size();
		if (!reverse) {
	...
	//push scrambled entries back onto passed in queue
	while(!q2.empty()) {
		T head = q2.front();
		q2.pop();
		q.push(head);
