...
void Allocator::loadRooms(const string& file)
{
	...
	roomCount = fileio::getNumRooms(); //added this line because roomCount is uninitialized and next line uses it
    ...
    while (fileio::areMoreRooms()) { 
		...
		i++; //move increment i to end of loop from start of loop to fix arrayoutofbounds error
    }
}
