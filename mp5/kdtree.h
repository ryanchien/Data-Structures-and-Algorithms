...
class KDTree
{
  ...
  private:
     ...
     */
		KDTreeNode * KDTreeHelper(vector<Point<Dim>>& vec, int left, int right, int dim);
		void quickSort(vector<Point<Dim>>& vec, int left, int right, int dim);
		int quickSortHelper(vector<Point<Dim>>& vec, int left, int right, int dim);
		void _destroy(KDTreeNode *& node);
		void _copy(const KDTree<Dim>& other);
		Point<Dim> findBest(const Point<Dim>& query, KDTreeNode* const & node, int dim) const;
};
