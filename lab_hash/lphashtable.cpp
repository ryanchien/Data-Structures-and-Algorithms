...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
	...
	pair<K,V> * elem = new pair<K,V>(key, value);
	// elem not found in idx -> insert
	if (table[idx] == NULL) {
		table[idx] = elem;
		should_probe[idx] = true;
		elems++;
	}
	// elem found in idx -> probe until empty cell found
	else {
		idx = (idx+1)%size;
...
void LPHashTable<K, V>::resizeTable()
{
	...
	for (size_t i=0; i<size; i++) {
		if (table[i] != NULL) {
			size_t newIdx = hash(table[i]->first, newSize);
			//resolving collisions in new table
